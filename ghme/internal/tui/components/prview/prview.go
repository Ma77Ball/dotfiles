...
func (m *Model) renderRequestedReviewers() string {
	...
	// ghme patch: rank reviewer state by strength, not recency, so each reviewer
	// icon reflects the strongest signal: APPROVED > CHANGES_REQUESTED > COMMENTED.
	reviewRank := map[string]int{"APPROVED": 3, "CHANGES_REQUESTED": 2, "COMMENTED": 1}
	for _, review := range reviews {
		login := review.Author.Login
		if reviewRank[review.State] > reviewRank[reviewStates[login]] {
