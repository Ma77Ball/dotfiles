...
func (issue *Issue) renderTag() string {
	tag := data.GetTagStore().Get(issue.Data.GetUrl())
	if tag == "" {
		return issue.Ctx.Styles.Common.FaintTextStyle.Render("")
	}
	return lipgloss.NewStyle().
		Foreground(issue.Ctx.Theme.SecondaryText).
		Render(tag)
}
