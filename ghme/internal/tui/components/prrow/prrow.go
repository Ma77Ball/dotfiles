...
func (pr *PullRequest) renderTag() string {
	if pr.Data == nil || pr.Data.Primary == nil {
		return pr.Ctx.Styles.Common.FaintTextStyle.Render("")
	}
	tag := data.GetTagStore().Get(pr.Data.GetUrl())
	if tag == "" {
		return pr.Ctx.Styles.Common.FaintTextStyle.Render("")
	}
	return lipgloss.NewStyle().
		Foreground(pr.Ctx.Theme.SecondaryText).
		Render(tag)
}
...
func (pr *PullRequest) renderReviewStatus() string {
	...
	// ghme patch: a re-requested / still-required review takes priority over the
	// "has comments" glyph, so PRs that need a review stand out (warning-colored
	// waiting glyph) instead of looking like they just have discussion on them.
	if pr.Data.Primary.ReviewDecision == "REVIEW_REQUIRED" {
		return reviewCellStyle.Render(pr.Ctx.Styles.Common.WaitingGlyph)
	}

	if pr.Data.Primary.Reviews.TotalCount > 0 {
		// ghme patch: GitHub only sets ReviewDecision when the repo requires
		// reviews, so an approval on an unprotected repo would otherwise fall
		// through to the plain comment glyph. Rank the actual reviews and show
		// the matching icon: APPROVED > CHANGES_REQUESTED > COMMENTED.
		rank := map[string]int{"APPROVED": 3, "CHANGES_REQUESTED": 2, "COMMENTED": 1}
		best := ""
		for _, review := range pr.Data.Primary.Reviews.Nodes {
			if rank[review.State] > rank[best] {
				best = review.State
			}
		}
		switch best {
		...
		case "CHANGES_REQUESTED":
			return reviewCellStyle.Foreground(pr.Ctx.Theme.ErrorText).Render(constants.ChangesRequestedIcon)
		}
...
func (pr *PullRequest) renderState() string {
	...
	case "OPEN":
		// ghme patch: an OPEN PR that conflicts with its base shows a red conflict
		// glyph in the state column so it stands out in the list.
