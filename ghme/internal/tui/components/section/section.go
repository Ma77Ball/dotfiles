...
type Section interface {
	...
	GetTotalCount() int
	// SetTagFilter narrows the section to items whose local tag matches filter
	// (empty clears it) and rebuilds the visible rows.
...
// HasFetched reports whether a fetch has been initiated for this section since
// it was built. ghme uses this to lazy-load a section's rows the first time
// it's viewed (LastFetchTaskId is set the moment FetchNextPageSectionRows runs).
func (m *BaseModel) HasFetched() bool {
	return m.LastFetchTaskId != ""
}
