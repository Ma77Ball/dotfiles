...
// tagInputMode tracks which (if any) single-line tag prompt is currently open
// in the footer.
type tagInputMode int

const (
	tagInputNone tagInputMode = iota
	tagInputSet               // typing a tag for the selected item
	tagInputFilter            // typing a tag to filter the current section by
)
...
type Model struct {
	...
	// Local tagging: a free-text label per PR/issue, persisted via data.TagStore.
	tagInput     textinput.Model
	tagMode      tagInputMode
	tagTargetURL string // item URL being tagged while tagMode == tagInputSet
}
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case (m.ctx.View == config.PRsView || m.ctx.View == config.IssuesView) &&
			key.Matches(msg, m.keys.SetTag):
			if currRowData == nil || reflect.ValueOf(currRowData).IsNil() {
				return m, m.notifyErr("Current selection isn't associated with a PR/Issue")
			}
			m.tagTargetURL = currRowData.GetUrl()
			m.tagMode = tagInputSet
			m.tagInput.SetValue(data.GetTagStore().Get(m.tagTargetURL))
			m.tagInput.CursorEnd()
			m.footer.SetLeftSection(m.tagInputView())
			return m, m.tagInput.Focus()

		case (m.ctx.View == config.PRsView || m.ctx.View == config.IssuesView) &&
			key.Matches(msg, m.keys.TagFilter):
			m.tagMode = tagInputFilter
			if currSection != nil {
				m.tagInput.SetValue(currSection.GetTagFilter())
			} else {
				m.tagInput.SetValue("")
			}
			m.tagInput.CursorEnd()
	...
	if currSection != nil && m.tagMode == tagInputNone {
...
// updateTagInput handles keystrokes while a tag prompt (set or filter) is open
// in the footer. Enter commits, Esc cancels; anything else edits the text.
func (m Model) updateTagInput(msg tea.Msg) (tea.Model, tea.Cmd) {
	if km, ok := msg.(tea.KeyMsg); ok {
		...
		case "enter":
			...
			mode := m.tagMode
			target := m.tagTargetURL
			m.closeTagInput()
			switch mode {
			case tagInputSet:
				data.GetTagStore().Set(target, val)
				if cs := m.getCurrSection(); cs != nil {
					cs.RefreshRows()
				}
			case tagInputFilter:
				if cs := m.getCurrSection(); cs != nil {
...
// closeTagInput tears down the tag prompt and clears the footer.
func (m *Model) closeTagInput() {
	m.tagMode = tagInputNone
	m.tagTargetURL = ""
	m.tagInput.Blur()
	m.tagInput.SetValue("")
	m.footer.SetLeftSection("")
...
// tagInputView renders the tag prompt for the footer's left section.
func (m Model) tagInputView() string {
	label := "tag"
	if m.tagMode == tagInputFilter {
		label = "filter tag"
	}
	prompt := m.ctx.Styles.ListViewPort.PagerStyle.Render(fmt.Sprintf("%s: ", label))
	return prompt + m.tagInput.View()
}
...
func (m *Model) onViewedRowChanged() tea.Cmd {
	var cmds []tea.Cmd
	// ghme patch: lazy-load. The first time a PR/Issue section is viewed, fetch
	// its rows (FetchAllSections only fetched the section that was active at
	// launch/refresh). HasFetched() guards against re-fetching.
	if m.ctx.View == config.PRsView || m.ctx.View == config.IssuesView {
		if cs := m.getCurrSection(); cs != nil && !cs.HasFetched() {
			cmds = append(cmds, cs.FetchNextPageSectionRows()...)
		}
	}
	...
	cmds = append(cmds, sidebarCmd, enrichCmd)
	return tea.Batch(cmds...)
}
...
func (m *Model) fetchAllViewSections() ([]section.Section, tea.Cmd) {
	...
	case config.PRsView:
		s, _ := prssection.FetchAllSections(m.ctx, m.prs)
		// ghme patch: lazy-load. Fetch only the active section now; the rest
		// load on first view (see onViewedRowChanged).
		cmds = append(cmds, m.fetchActiveSectionRows(s)...)
		return s, tea.Batch(cmds...)
	default:
		s, _ := issuessection.FetchAllSections(m.ctx)
		// ghme patch: lazy-load. Fetch only the active section now; the rest
		// load on first view (see onViewedRowChanged).
		cmds = append(cmds, m.fetchActiveSectionRows(s)...)
		return s, tea.Batch(cmds...)
...
// fetchActiveSectionRows returns the fetch commands for whichever freshly-built
// section matches the current section id, so lazy-load only pays for the tab
// the user is actually looking at. ghme patch.
func (m *Model) fetchActiveSectionRows(sections []section.Section) []tea.Cmd {
	for _, sec := range sections {
		if sec != nil && sec.GetId() == m.currSectionId {
			return sec.FetchNextPageSectionRows()
		}
	}
	return nil
}
